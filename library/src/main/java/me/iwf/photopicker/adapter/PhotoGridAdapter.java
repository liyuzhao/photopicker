...
public class PhotoGridAdapter extends SelectableAdapter<PhotoGridAdapter.PhotoViewHolder> {
  ...
  @Override public void onBindViewHolder(final PhotoViewHolder holder, final int position) {
    ...
    if (getItemViewType(position) == ITEM_TYPE_PHOTO) {
      ...
      Glide.with(mContext).load(new File(photo.getPath()))
              .apply(RequestOptions.placeholderOf(R.drawable.ic_photo_black_48dp)
                      .error(R.drawable.ic_broken_image_black_48dp).centerCrop()).thumbnail(0.1f).into(holder.ivPhoto);
